...
class MyForm(QDialog):
    ...
    def result1(self):
        if len(self.ui.lineEditBookPrice.text()):
            bookPrice = eval(self.ui.lineEditBookPrice.text())
        else:
            bookPrice = 0
        totalBookAmount = self.ui.spinBoxBookQty.value() * bookPrice
        return self.ui.lineEditBookAmount.setText(str(totalBookAmount))

    def result2(self):
        if len(self.ui.lineEditSugarPrice.text()):
            sugarPrice = eval(self.ui.lineEditSugarPrice.text())
        else:
            sugarPrice = 0
        ...
        totalBookAmount = float(self.ui.lineEditBookAmount.text())
